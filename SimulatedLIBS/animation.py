...
def update_plot(index, parameter, wavelength_range, intensity_range, parameter_range, unit):
    plt.clf()
...
def animate_resolution(elements, percentages):
    ...
    wavelength_range, intensity_range = get_intensity(resolution_range=resolution_range, Te_range=[1.0] * anim_len,
                                                      Ne_range=[10 ** 17] * anim_len, elements=elements,
                                                      percentages=percentages, low_w=200, upper_w=400)

    fig = plt.figure()
    anim = animation.FuncAnimation(fig, update_plot, frames=anim_len,
                                   fargs=('Resolution', wavelength_range, intensity_range, resolution_range, ' '),
                                   interval=200)
    anim.save(os.path.join('animations', 'saved-gifs', 'animated_resolution.gif'), writer='imagemagick')
    plt.show()
...
def animate_temperature(elements, percentages):
    ...
    wavelength_range, intensity_range = get_intensity(resolution_range=[5000] * anim_len, Te_range=Te_range,
                                                      Ne_range=[10 ** 17] * anim_len, elements=elements,
                                                      percentages=percentages, low_w=200, upper_w=400)
    ...
    anim = animation.FuncAnimation(fig, update_plot, frames=anim_len,
                                   ...
                                   interval=200)
    anim.save(os.path.join('animations', 'saved-gifs', 'animated_temperature.gif'), writer='imagemagick')
    plt.show()
...
def animate_density(elements, percentages):
    ...
    wavelength_range, intensity_range = get_intensity(resolution_range=[5000] * anim_len, Te_range=[1.0] * anim_len,
                                                      Ne_range= Ne_range, elements=elements,
                                                      percentages=percentages, low_w=200, upper_w=400)
    ...
    anim = animation.FuncAnimation(fig, update_plot, frames=anim_len,
                                   ...
                                   interval=200)
    anim.save(os.path.join('animations', 'saved-gifs', 'animated_density.gif'), writer='imagemagick')
    plt.show()
